...
export const openai = new OpenAI({
    ...
    defaultHeaders: {
        "HTTP-Referer": "https://your-site-url.vercel.app",
...
6. Travel interests (e.g., adventure, sightseeing, cultural, food, nightlife, relaxation) 7. Special requirements or preferences (if any)

Do not ask multiple questions at once, and never ask irrelevant questions. If any answer is missing or unclear, politely ask the user to clarify before proceeding.

Always maintain a conversational, interactive style while asking questions.

Along with response also send which ui component to display for generative Ul for example (budget/groupSize/TripDuration/final), where Final means Al generating complete final output Once all required information is collected, generate and retum a strict JSON response only (no explanations or extra text) with following JSON schema:
{
resp:Text Resp',
...
export async function POST(req: NextRequest) {
    const { message, isFinal } = await req.json();
    try {
